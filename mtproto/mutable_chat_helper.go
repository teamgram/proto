...
func (m *MutableChat) ChatPhoto() *ChatPhoto {
	return MakeChatPhotoByPhoto(m.GetChat().GetPhoto())
}

func (m *MutableChat) Photo() *Photo {
...
func (m *MutableChat) MigratedTo() *InputChannel {
	return m.GetChat().GetMigratedTo()
}

func (m *MutableChat) DefaultBannedRights() *ChatBannedRights {
...
func (m *MutableChat) Call() *InputGroupCall {
	return m.GetChat().GetCall()
}

func (m *MutableChat) AvailableReactions() *ChatReactions {
	return FromChatReactions(m.GetChat().GetAvailableReactionsType(), m.GetChat().GetAvailableReactions())
}
...
func (m *MutableChat) MakeMessageService(fromId int64, action *MessageAction) *Message {
...
func (m *MutableChat) ToUnsafeChat(id int64) *Chat {
	var (
		ok bool
	...
	chat := &Chat{
		...
		Id:                      m.Id(),
		Title:                   m.Title(),
		Photo:                   MakeChatPhotoByPhoto(m.Chat.Photo),
	...
	if m.Deactivated() {
		...
		return MakeTLChat(chat).To_Chat()
	}

	if me, ok = m.GetImmutableChatParticipant(id); !ok {
		// TODO(@benqi): ???
		return MakeTLChatEmpty(chat).To_Chat()
	}

	if me.IsChatMemberStateKicked() {
		return MakeTLChatForbidden(chat).To_Chat()
	}
...
func (m *MutableChat) ToChat() *Chat {
	chat := &Chat{
		Creator:                 false,
		Left:                    false,
		Deactivated:             false,
...
func (m *MutableChat) ToChatParticipants(selfId int64) (participants *ChatParticipants) {
	if selfId != 0 {
		if me, ok := m.GetImmutableChatParticipant(selfId); ok && !me.IsChatMemberStateNormal() {
			participants = MakeTLChatParticipantsForbidden(&ChatParticipants{
				ChatId:          m.Chat.Id,
	...
	participants = MakeTLChatParticipants(&ChatParticipants{
		ChatId:       m.Chat.Id,
		Participants: make([]*ChatParticipant, 0, len(m.ChatParticipants)),
...
func (m *MutableChat) ToChatForbidden() (chat *Chat) {
	chat = MakeTLChatForbidden(&Chat{
		Id:    m.Chat.Id,
		Title: m.Chat.Title,
	}).To_Chat()
	return
