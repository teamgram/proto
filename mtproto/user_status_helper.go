...
var (
	userStatusEmpty = MakeTLUserStatusEmpty(nil).To_UserStatus()
)

func MakeUserStatusOnline() *UserStatus {
	now := time.Now().Unix()
	status := MakeTLUserStatusOnline(&UserStatus{
		Expires: int32(now + 60),
	}).To_UserStatus()
	return status
}

func MakeUserStatusOffline(lastSeenAt int64) *UserStatus {
	return MakeTLUserStatusOffline(&UserStatus{
...
/**********************************************************************************
	...
	// 60*60*24*30  month
*/
func MakeUserStatus(lastSeenAt int64, allowTimestamp bool) *UserStatus {
	now := time.Now().Unix()
	...
	if allowTimestamp {
		if now <= lastSeenAt+60 {
			...
			status := MakeTLUserStatusOffline(&UserStatus{
				WasOnline: int32(lastSeenAt),
			}).To_UserStatus()
			return status
		}
		...
		if now-lastSeenAt >= 60*60*24*30 {
			...
			return MakeTLUserStatusLastMonth(nil).To_UserStatus()
		} else if now-lastSeenAt >= 60*60*24*3 {
			return MakeTLUserStatusLastWeek(nil).To_UserStatus()
		} else {
			return MakeTLUserStatusRecently(nil).To_UserStatus()
		}
	}
}
